...
# 서버에 있는 폴더 분류할 때
def detect_eyes2(extractFolder):
    # EAR 계산 함수 (눈 감김 상태를 판별하기 위한 함수)
    def calculate_EAR(eye_points):
        A = np.linalg.norm(eye_points[1] - eye_points[5])
        B = np.linalg.norm(eye_points[2] - eye_points[4])
    ...
    file_name = "Eyes_Detection_" + extractFolder           # Eye_Detection_대전_test1
    file_name2 = "No_Eyes_Detection_" + extractFolder       # No_Eyes_Detection_대전_test1
    resultFolderPath = './ClassifyResult/'
    result_eyes_folder = resultFolderPath + file_name       # ./ClassifyResult/Eye_Detection_대전_test1
    result_no_eyes_folder = os.path.join(resultFolderPath, file_name2)  # ./ClassifyResult/Eye_Detection_대전_test1
